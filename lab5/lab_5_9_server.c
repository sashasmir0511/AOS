...
void	work_server(t_server *q_s, t_client *q_c, int server_id, int client_id)
{
	while (1)
	{
		...
		msgsnd(client_id, q_c, sizeof(q_c), 0);
	}
}
...
int		main(int argc, char **argv)
{
	...
	if (argc != 3)
	{
		creat("server9", 0777);
		creat("client9", 0777);
		if ((key1 = ftok("server9", 'A')) == -1
			|| (key2 = ftok("client9", 'A')) == -1)
		{
			perror("ftok");
			exit(1);
		}
		if ((server_id = msgget(key1, IPC_CREAT | 0600)) == -1
			|| (client_id = msgget(key2, IPC_CREAT | 0600)) == -1)
		{
			perror("msgget");
			exit(1);
		}
	...
	work_server(q_s, q_c, server_id, client_id);
	exit(0);
}
